...
- 의존 오브젝트의 변경을 통한 테스트 방법
    - 테스트 환경에서 `UserDao`에서 사용하는 운영용 DB 대신 설정이 간편한 경량 DB를 이용할 수 있다.
    - `UserService` 테스트 시, 실제 메일 서버 대신 `DummyMailSender`를 통해 테스트가 원할하게 진행되도록 할 수 있다.
    - 운영 환경에서 구현 클래스가 바뀌지 않는다고 하더라도, 테스트를 위해서라도 스프링의 DI를 적용해야 한다. 
