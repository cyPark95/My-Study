...
- `@Component`를 이용한 자동 빈 등록(소스 참고)
    ...
    - 여러 어노테이션에 공통 속성을 부여하려면 메타 어노테이션을 사용할 수 있다.
        - 어노테이션의 정의에 부여된 어노테이션을 메타 어노테이션이라고 한다.
    - 스프링에서 계층별 권장 어노테이션
        - `@Repository`: DAO 기능 제공 클래스
        - `@Service`: 비즈니스 로직을 담고 있는 서비스 빈
...
- `SqlService` 관련 설정은 다른 빈 설정과 성격이 다르기 때문에 분리함으로써 여러 프로젝트에서 재사용할 수 있다.
- 빈 설정자(소스 참고)
    - `SqlService`는 sqlmap 파일 위치에 종속적이므로, 독립적인 모듈로 재사용하기 위해 설정을 별도로 분리해야 한다.
    - sqlmap 파일의 위치는 초기 설정 시 한 번만 지정하면 되므로, 이를 인터페이스로 분리하고 DI를 통해 주입받는 방식으로 구성할 수 있다.
    - `AppContext` 역시 빈이기 때문에 인터페이스를 직접 구현하여 자기 참조 빈을 만들 수 있다.
- `@Enable..` 어노테이션(소스 참고)
    - 빈 자동 등록 시에는 `@Component`보다 `@Repository`나 `@Service`와 같이 의미 있는 어노테이션을 사용하는 것을 권장한다.
        - 계층을 나타낼 수 있다.
        - AOP를 통해 특정 어노테이션이 달린 빈만 부가 기능을 제공할 수 있다.
    - 모듈화된 빈 설정을 사용할 경우, `@Enable`로 시작하는 어노테이션을 통해 해당 설정을 활성화할 수 있다.
        - `@Enable..` 어노테이션은 해당 설정을 활성화 한다는 의미를 가지고 있다.
        - 내부적으로는 `@Import`를 메타 어노테이션으로 포함하고 있다.

## 7.7 정리

- 스프링을 가치 있게 활용하려면 객체지향 설계를 효과적으로 적용할 줄 알고, 스프링이 제공하지 않는 기능을 구현할 때도 서비스 추상화와 AOP 등을 활용할 수 있어야 한다.
...
- 자주 사용되는 의존 객체는 디폴트로 미리 정의해두면 편리하다.
- XML과 객체 매핑은 스프링의 OXM 추상화 기능을 활용한다.
...
- DI에는 인터페이스를 시용한다. 인터페이스를 시용하면 인터페이스 분리 원칙을 잘 지키는 데도 도움이 된다.
- 클라이언트에 따라서 인터페이스를 분리할 때, 새로운 인터페이스를 만드는 방법과 인터페이스를 상속하는 방법 두 가지를 사용할 수 있다.
- 애플리케이션에 내장하는 DB를 사용할 때는 스프링의 내장형 DB 추상화 기능과 전용 태그를 시용하면 편리하다.
